...
{
    partial class RequestGetHostInfoHandler : AbstractRequestHandler
    {
        public override void Handle(SocketSession session, ePacketType reqType, object reqObj)
        {
            RunTaskThread(() =>
            {
                ...
                try
                {
                    ...
                    CollectInstalledSoftware(resp.InstalledSoftware);
                    CollectSecuritySoftware(resp.SecuritySoftware);
                    resp.Result = true;
                }
                catch (Exception ex)
                {
        ...
        private HostHardwareInfo CollectHardwareInfo()
        {
            ...
            try
            {
                ...
                info.TotalMemoryMB = (long)(mem.TotalPhysicalMemory / (1024 * 1024));
                info.AvailableMemoryMB = (long)(mem.AvailablePhysicalMemory / (1024 * 1024));
            }
            catch
            {
                try
